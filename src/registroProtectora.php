...
<!-- crear protectora -->
  <?php 
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') 
    {
      ...
      $Activa = 0;
      $stmt = $bd->prepare("INSERT INTO Protectoras (Correo, Nombre, Ubicacion, Contrasena, Activa) VALUES (?, ?, ?, ?, ?)");
      try{$stmt->execute([$Correo, $Nombre, $Ubicacion, $ContrasenaHash, $Activa]);
      
      $imagen = $_FILES['imagen']['name'] ?? '';
      $protectora_id = $bd->lastInsertId();

       // Procesar la imagen
        $ruta_imagen = '';
        if (isset($_FILES['imagen']) && $_FILES['imagen']['error'] == 0) {
            // Definir la carpeta donde se guardarán las imágenes
            $carpeta_destino = "multimedia/protectoras/" . $protectora_id . "/perfil";
            ...
            if (move_uploaded_file($_FILES['imagen']['tmp_name'], $ruta_completa)) {
                $ruta_imagen = "multimedia/protectoras/" . $protectora_id . "/perfil/" . $nombre_archivo;
            } 
        }
  ...
  <main class="flex flex-col justify-center items-center w-full gap-4 relative pt-[50px] p-2">
    ...
    <input type="number" id="Telefono" name="Telefono" required class="w-full p-2 mb-4 border border-gray-600 rounded bg-gray-700 text-white focus:outline-none focus:ring-2 focus:ring-blue-500">
